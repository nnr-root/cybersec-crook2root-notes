...
tags: [tree/tooling, cyber/tooling/development/shadowstep, type/concept, level/root]
...
- **Crook:** Why does ShadowStep promote the scope guard to a mandatory gate, and what does the dry-run plan produce?
- **Operator:** Walk the plan → simulate → verify flow, and explain what each guarantees.
- **Root:** Explain why deleting a log doesn't destroy the evidence, and how ShadowStep pairs each action with its detection.
